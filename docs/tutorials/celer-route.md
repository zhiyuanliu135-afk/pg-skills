...
| 验证合并 | build 完成后明确要求“verify 并合并”，加载 `pg-verify-and-merge` | 验证已归档的变更，并把功能分支合并到默认分支 |
...
## 可选：使用 Auto-Pilot 完成下一次任务

前面的步骤展示了需要保留完整 Spec 和执行记录的标准流。pg-skills v0.9.3 新增了 `/0-pg-auto-pilot`：对于范围明确、预计一次会话可以完成的实现任务，可以让 AI 自主决定如何规划和执行，不必先生成 `proposal.md`、`design.md` 和 `tasks.md`。

Auto-Pilot 并不是“跳过验证”。它仍然有两条强制要求：

1. 执行计划中必须包含“启动实例并验证编码结果是否达到预期”；
2. 执行计划前必须让用户选择环境，并确认需要准备环境，还是环境已经就绪。

### 什么时候使用 Auto-Pilot

适合使用 Auto-Pilot 的任务通常具有以下特点：

- 目标和修改范围已经明确；
- 可以在一次 OpenCode 会话中完成；
- 不要求保留完整的 proposal、design 和 tasks 变更文档；
- 仍然需要在项目环境中启动或重启实例，并验证实际运行结果。

如果任务涉及多个模块、复杂设计决策或需要完整审计记录，仍应使用前文的 define → propose → build 标准流。Auto-Pilot 也不负责合并代码；需要合并时，仍应由用户明确发出验证合并指示。

### 在 OpenCode 中发送任务

例如，完成本教程的报表功能后，可以使用 Auto-Pilot 继续完善下载失败时的用户提示。在 OpenCode 中一次发送：

```text
/0-pg-auto-pilot

请为 celer-route 日志页面的“导出报表”操作增加下载失败提示。

要求：
1. 复用项目现有的消息提示组件，不新增通知框架。
2. 不修改现有 CSV 接口和成功下载流程。
3. 补充对应的前端测试。
4. 修改完成后启动或重启 UI 实例，并在真实页面中验证失败提示。
```

`/0-pg-auto-pilot` 后面的内容就是本次实现任务。命令加载 `pg-auto-pilot` skill 后，AI 会自行调查代码、形成计划并选择实现和验证手段。

### 确认环境并检查结果

正式执行计划前，Auto-Pilot 会要求确认：

1. 使用哪个环境，例如本教程配置的 `local`；
2. 是否需要运行 `prepare_env`，或者环境已经就绪，可以跳过准备。

确认后，AI 才会执行计划。代码修改完成后，即使实例此前已经运行，也必须通过项目 hooks 重启实例或执行 stop + start，并在 `health_check` 通过后验证页面行为。验证失败时应继续修复和重验；无法恢复时则向用户报告。

完成报告至少应说明代码修改、测试与运行时验证结果，以及 hook 日志位置。确认结果符合预期后，再由用户决定是否明确要求验证并合并。

## 后续学习

本教程已经介绍标准流和 Auto-Pilot 自动驾驶流。接下来可以尝试其他专项工作流：
...
- [pg-skills 完整介绍](https://github.com/pin-gou/pg-skills/blob/main/docs/pg-skills.md)：了解 SEA 方法论、标准流程、快捷流和新项目接入方式。
- [开发工具适配器说明](https://github.com/pin-gou/pg-skills/blob/main/docs/tool-integrations.md)：了解 pg-skills 如何接入 OpenCode 等开发工具。

遇到初始化、命令加载或工作流中断时，可以先返回 [pg-skills 首页](https://pin-gou.github.io/pg-skills/) 查阅常见问题与速查卡。
